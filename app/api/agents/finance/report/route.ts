  ...
  try {
    ...
    const signalRows = signals.length
      ? signals
          .map(
            (s) =>
              `  [${s.action?.toUpperCase()}] ${s.product_name} — ${s.reason}`,
          )
          .join("\n")
      : "  No active signals.";
    ...
    const bottomProduct = [...byProduct].sort(
      (a: ProductMetric, b: ProductMetric) =>
        parseFloat(a.margin_pct) - parseFloat(b.margin_pct),
    )[0];
    const repriceProducts = byProduct.filter(
      (p: ProductMetric) => parseFloat(p.margin_pct) < 30,
    );
    const boostProducts = byProduct.filter(
      (p: ProductMetric) => parseFloat(p.margin_pct) >= 40,
    );

    const fallbackObs = [
      topProduct
        ? `${topProduct.title} leads with a ${parseFloat(topProduct.margin_pct).toFixed(1)}% margin and ${topProduct.units_sold} units sold, making it the strongest performer in the portfolio.`
        : "",
      bottomProduct && parseFloat(bottomProduct.margin_pct) < 20
        ? `${bottomProduct.title} is critically underperforming at ${parseFloat(bottomProduct.margin_pct).toFixed(1)}% margin — immediate repricing or retirement is recommended.`
        : "",
      repriceProducts.length > 0
        ? `${repriceProducts.length} product(s) fall below the 30% margin threshold and require pricing review.`
        : "All products are maintaining healthy margins above the 30% target.",
    ]
      .filter(Boolean)
      .join(" ");
    ...
    const fallbackRec = [
      boostProducts.length > 0
        ? `1. Increase ad spend on ${boostProducts.map((p: ProductMetric) => p.title).join(" and ")} — high margin makes them strong ROI candidates.`
        : "1. Identify top-margin products and allocate promotional budget accordingly.",
      repriceProducts.length > 0
        ? `2. Raise prices on ${repriceProducts.map((p: ProductMetric) => p.title).join(", ")} to bring margins above 30%.`
        : "2. Maintain current pricing strategy as all products are above margin targets.",
...
Output ONLY the 3 sections separated by "|||". No titles, no intro text, no markdown.`,
    ...
    const productTableRows = byProduct
      ...
      .map((p) => {
        ...
        const marginColor =
          m >= 40 ? "#2D7A4F" : m >= 25 ? "#D97706" : "#C0584A";
...
<body style="margin:0;padding:0;font-family:system-ui,-apple-system,sans-serif;background:#F7F6F2;color:#141412;">
  ...
  <div style="padding:40px 48px;max-width:960px;margin:0 auto;">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:40px;">
      ${[
        ...
        { label: "Printify Costs", value: `RM ${costs}`, color: "#C0584A" },
        {
          label: "Net Profit",
          value: `RM ${profit}`,
          color: parseFloat(profit) >= 0 ? "#2D7A4F" : "#C0584A",
        },
        {
          label: "Profit Margin",
          value: `${margin}%`,
          color: parseFloat(margin) >= 30 ? "#2D7A4F" : "#D97706",
        },
      ]
        .map(
          (k) => `
    ...
    const browser = await puppeteer.launch({
      args: chromium.args,
      defaultViewport: (
        chromium as { defaultViewport?: { width: number; height: number } }
      ).defaultViewport,
      executablePath,
      headless: true,
    });

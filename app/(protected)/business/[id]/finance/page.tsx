                ...
                <div className='bg-[#FAFAF8] border border-[#E8E7E2] rounded-[12px] p-[20px] h-[190px] flex flex-col'>
                  ...
                  <div className='flex items-end gap-[4px] flex-1 mt-[10px]'>
                    {[45, 55, 48, 62, 58, 70, parseFloat(summary?.overall_margin_pct) || 82].map((h, i) => (
                      <div
                        key={i}
                        className={`flex-1 rounded-[4px_4px_0_0] ${i === 6 ? 'bg-gradient-to-t from-[#141412] to-[#2A2A27]' : 'bg-gradient-to-t from-[#9E7A2E] to-[#C9A84C] opacity-60'} transition-opacity delay-75`}
                        style={{ height: `${h}%` }}
                      />
                    ))}
                  </div>
                      ...
                      <div key={idx} className='flex gap-[12px] items-start p-[10px] bg-[#F4F3EF] rounded-[8px]'>
                        ...
                        <div>
                          <div className='text-[12px] font-semibold text-[#141412]'>
                            {sig.action}
                            <span className='font-medium text-[#6B6A64] mx-[4px]'>•</span>
                            <span className='font-medium text-[#141412]'>{sig.product_name || 'Product'}</span>
                          </div>
                          <div className='text-[11px] text-[#6B6A64] mt-[4px] leading-snug'>
                            {sig.reason || 'Margin threshold triggered based on recent sales.'}
                          </div>
                        </div>
                      </div>
                    )) : (
                ...
                <div className='flex-1 overflow-y-auto pr-[4px] flex flex-col gap-[12px] pb-[10px] [&::-webkit-scrollbar]:w-[4px] [&::-webkit-scrollbar-thumb]:bg-[#FAFAF8]/20 [&::-webkit-scrollbar-thumb]:rounded-full'>
                  {messages.map((msg, i) => (
                    <div key={i} className={`flex gap-[10px] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                      {msg.role === 'ai' ? (
                        <div className='w-[24px] h-[24px] rounded-full bg-[#141412] flex items-center justify-center text-[9px] shrink-0 font-semibold'>FA</div>
                      ) : (
                        <div className='w-[24px] h-[24px] rounded-full bg-[#C9A84C] flex items-center justify-center text-[9px] shrink-0 font-semibold text-[#FAFAF8]'>U</div>
                      )}
                ...
                <table className='w-full'>
                  ...
                  <tbody>
                    {products.length > 0 ? products.map((p: any) => (
                      <tr key={p.product_id} className='hover:bg-[#F4F3EF] border-b border-[#E8E7E2] last:border-b-0'>
                        <td className='px-[20px] py-[12px] text-[13px]'>#{p.product_id?.slice(-4) || '—'}</td>
                        ...
                        <td className={`px-[20px] py-[12px] text-[13px] font-semibold ${p.profit < 0 ? 'text-[#C0584A]' : 'text-[#2D7A4F]'}`}>

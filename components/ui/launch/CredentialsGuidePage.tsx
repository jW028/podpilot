  ...
  return (
    <section className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="font-serif text-3xl font-bold text-dark">Launch Credentials Guide</h1>
      ...
      <div className="rounded-xl border border-neutral-300 bg-white p-5 space-y-6">
        <div>
          <h2 className="font-serif text-xl text-dark mb-2">Step 1: Create Printify API Token</h2>
          <p className="text-sm text-neutral-600">
            In Printify dashboard, open <strong>My Profile</strong> → <strong>Connections</strong>{" "}
            and create a Personal Access Token with upload/product scopes.
          </p>
        </div>

        <div>
          <h2 className="font-serif text-xl text-dark mb-2">Step 2: Get Your Shop ID</h2>
          <p className="text-sm text-neutral-600">
            Fetch your shops using the Printify API and copy the numeric shop ID for the business.
          </p>
        </div>

        <div>
          <h2 className="font-serif text-xl text-dark mb-2">Step 3: Configure Required Env Keys</h2>
...
GEMINI_MODEL=gemini-2.5-flash`}
        ...
        <div>
          <h2 className="font-serif text-xl text-dark mb-2">Step 4: Add Placeholder Image</h2>
          <p className="text-sm text-neutral-600">
            Place your launch artwork in <code>assets/printify-placeholders</code>. By default the
            launch flow reads <code>images.png</code> unless <code>PRINTIFY_PLACEHOLDER_IMAGE</code>{" "}
            is set.
          </p>
        </div>

        <div>
          <h2 className="font-serif text-xl text-dark mb-3">Step 5: Pre-Launch Checklist</h2>
          <ul className="space-y-2 text-sm text-neutral-600">
            ...
            <li className="inline-flex items-start gap-2">
              <FiCheckSquare className="mt-0.5 text-primary-700 shrink-0" />
              Tavily and model keys are set so market research and pricing can run.
            </li>
          </ul>
        </div>
      </div>

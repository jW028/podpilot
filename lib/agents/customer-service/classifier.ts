...
const CLASSIFICATION_PATTERNS: ClassificationPattern[] = [
  ...
  },
  {
    ...
    patterns: [
      ...
      /\b(my\s+)?(parcel|package|order|item)\s+(is\s+)?(lost|missing|not\s+here)\b/i,
      /\b(haven'?t|have\s+not)\s+(got|received|gotten)\s+(my\s+)?(order|parcel|package|item)\b/i,
    ],
    keywords: [
      "never arrived", "didn't arrive", "missing", "lost package",
      "not received", "lost parcel", "parcel lost", "is lost",
      "hasn't arrived", "not arrived", "where is my",
    ],
    confidence: 0.93,
  },
  {
